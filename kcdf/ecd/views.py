...
def page (request,slug=""):
	...
	page=Page.objects.get(slug__contains='about-ecd')
	children=Page.objects.all().filter(parent=page)
	context_dict={'active_tab': 'about-ecd',"program":program,"children":children}
...
def resources(request):
	...
	return render_to_response('ecd/resources.html',context_dict,context_instance=RequestContext(request));

def resource_detail(request,slug):
	resource = get_object_or_404(Resource, slug=slug)
	context_dict={'active_tab': 'resource',"resource":resource}
	return render_to_response('ecd/resource_detail.html',context_dict,context_instance=RequestContext(request));
...
def news_detail(request,slug):
	...
	context_dict={'active_tab': 'news',"news":news}
	return render_to_response('ecd/news_detail.html',context_dict,context_instance=RequestContext(request));

def events(request):
	p=Program.objects.get(slug__contains='childhood')
	events=Events.objects.filter(program=p).order_by("-created_at")
	context_dict={'active_tab': 'events',"events":events}
	return render_to_response('ecd/events.html',context_dict,context_instance=RequestContext(request));

def events_detail(request,slug):
	events = get_object_or_404(Events, slug=slug)
	context_dict={'active_tab': 'events',"events":events}
	return render_to_response('ecd/events_detail.html',context_dict,context_instance=RequestContext(request));
...
def case_studies (request):
	...
	return render_to_response('ecd/casestudies.html',context_dict,context_instance=RequestContext(request));
